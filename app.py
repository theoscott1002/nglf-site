...
def create_app():
    ...
    def apply():
        if request.method == "POST":
            ...
            if errors:
                for e in errors:
                    flash(e, "error")
                return render_template("apply.html", title="Apply", form=request.form)
            ...
            # Email notify admin (safe no-op if SMTP not configured)
    ...
    def admin_applications():
        conn = get_connection()
        rows = conn.execute("""
            SELECT id, full_name, email, phone, city, french_level,
                   university_status, motivation, created_at
            FROM applications
            ORDER BY id DESC
        """).fetchall()
        conn.close()
    ...
    def admin_export_csv():
        ...
        writer.writerow([
            ...
            "french_level", "university_status", "motivation", "created_at"
        ])
        for r in rows:
            writer.writerow([
                r["id"], r["full_name"], r["email"], r["phone"], r["city"],
                r["french_level"], r["university_status"], r["motivation"], r["created_at"]
            ])
